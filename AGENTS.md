...
- **KingOfShojo — per-card detail fetch in `buildFeaturedItems`.** The [no per-item detail fan-out](#no-per-item-detail-fan-out) rule is violated here. Its original justification — the site's "Popular Today" listing carries no adult signal and per-item adult filtering was required — **no longer applies**: the source is now declared `ADULT` with no show-adult toggle, so nothing is filtered out. The fetch survives only to enrich each featured card with author, synopsis and status, which is not on its own a licence to fan out. Treat it as debt to remove, not as a sanctioned deviation, and do not carry it into the themed port. [review]
