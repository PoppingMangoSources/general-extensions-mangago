...
- **Name a section handler `get<Name>Section()` or `get<Name>SectionItems()` — both are merged, pick one per source.** niclimcy uses `getMangaListSection`/`getGenresSection` in MangaFire and `getFilteredSectionItems`/`getGenreSectionItems` in Mgeko; HiveToons uses the `SectionItems` form throughout. There is no single standard here, so match the source you are following and stay consistent within the file. Mark them `private`.
- **Keep the dispatcher itself thin.** MangaFire's `getDiscoverSectionItems` is a switch whose every arm delegates to `getMangaListSection` or `getGenresSection`, both parameterised so six sections share two handlers. Parameterise a handler across sections that differ only by a sort token, card type or option list rather than writing one method per section.
