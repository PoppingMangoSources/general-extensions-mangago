...
export const parseMangaDetails = async ($: CheerioAPI, mangaId: string): Promise<SourceManga> => {
  ...
  const isAdultContent = $('strong:contains("Adult Content")').next().text().trim();
...
export const parseChapters = ($: CheerioAPI, sourceManga: SourceManga): Chapter[] => {
  ...
  for (const chapterObj of arrChapters) {
    ...
    const publishDate = new Date($("time.opacity-50", chapterObj).attr("datetime") ?? "");
    const title = $("span.grow.flex.gap-2 span", chapterObj).first().text().trim();
    ...
    if (matches && matches[matches.length - 1]) {
      ...
      chapterType = title.slice(0, -matches[matches.length - 1].length).trim();
    }
    sortingIndex--;
  ...
  if (totalTypes > 1) {
    for (const chapter of chapters) {
      chapter.volume = totalTypes + chapterTypeToIdMap[chapter.chapterType];
    }
  }
...
export const parseRecommendedSection = async ($: CheerioAPI): Promise<DiscoverSectionItem[]> => {
  ...
  for (const recommendationObj of $(".glide__slide:not(.glide__slide--clone)").toArray()) {
    const id =
      $("a", recommendationObj).attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
...
export const parseRecentSectionViewMore = async ($: CheerioAPI): Promise<DiscoverSectionItem[]> => {
  const recentSectionArray: DiscoverSectionItem[] = [];
...
const parseRecentObject = (recentObj: Element, $: CheerioAPI): DiscoverSectionItem => {
  const id =
    $("a.aspect-square", recentObj).attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
  const chapterId =
    $("a.min-w-0", recentObj).attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
  const title = $("div.font-semibold", recentObj).text().trim() ?? "";
  const imageUrl =
    $("a img", recentObj).attr("src") ?? $("a img", recentObj).attr("data-src") ?? "";
...
export const parseHotSection = async ($: CheerioAPI): Promise<DiscoverSectionItem[]> => {
  const hotSectionArray: DiscoverSectionItem[] = [];
  for (const hotObj of $("article.flex.gap-4", "section.bg-base-200.max-w-7xl").toArray()) {
    const id =
      $("a", hotObj).first().attr("href")?.replace(/\/$/, "")?.split("/").slice(-2)[0] ?? "";
